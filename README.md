...
| `src/llm.py` | Gemini provider + fake provider for tests |
...
- **lexical** — Jaccard overlap on tokens (keys like `employer` still match "where do I work?" weakly; values match directly) plus keyword boost for direct word hits
- **salience** — extractor's importance score (0–1)
- **decay** — see below
...
> **Note:** General "liked X" preferences (e.g., "I like banana shake") never supersede "favorite X" memories (e.g., "favorite drink: coffee") because the extractor assigns them different keys. Only explicit replacement language ("my new favorite is...") triggers supersession.
...
4. Instructions to prioritize memory facts when answering user questions
5. Last ~16 conversation turns
...
- **Chroma / FAISS / Pinecone** — overkill; candidate set is small;.
...
- Fake provider replies are obviously canned; live personality needs `GEMINI_API_KEY`.
...
> If PowerShell shows a script execution error, run this first:
> ```powershell
...
Edit `.env` and set your free Gemini API key from [Google AI Studio](https://aistudio.google.com/app/apikey):
...
2. Several unrelated turns ("How was your morning?", weather, dinner, etc.).
...
2. Later: `I left Microsoft and joined Google.`
3. `Where do I work?` → **Google**.
